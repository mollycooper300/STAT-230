...
> RESPONSE: A data frame with 1846 rows and 3 columns. The variables included are: dataset, x-values, y-values. Added answer for Ex 1. 
...
> RESPONSE: 
 Correlation Coefficient = -0.06447185	
 This correlation implies that x and y have a weak negative correlation, probably statistically insignificant, in the dino dataset. 
...
> RESPONSE: Now I notice that the correlation between x and y seems to have little linear consistancy... it's a dinosaur!
...
> RESPONSE: R: -0.0629611	The correlation coefficient in the star data set is almost exactly the same as the dino data set, a very weak but slightly negative correlation between the x and y variables. 
...
> RESPONSE: The plot does not look the same as for the dino data, now it's in the shape of a star!

```{r fig.fullwidth=TRUE}
ggplot(data = star_data, mapping = aes(y = y, x = x)) +
  geom_point()
...
> RESPONSE: R = -0.06834336	
Again, the correlation is almost exactly the same as for star and dino, a very slight negative correlation. 
...
> RESPONSE: Nope! As true to the other two data sets, the circle data set shape matches it's name in the shape of a circle!

```{r fig.fullwidth=TRUE}
ggplot(data = circle_data, mapping = aes(y = y, x = x)) +
  geom_point()
```
...
> RESPONSE: I think the correlation coefficiant is certainly an appropriate summary for all of these data sets given how minute the distinctions in their coefficients are. They are all almost exactly a -.06 correlation which to me shows them all to be fairly similar in this trait. 
