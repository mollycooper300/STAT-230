...
> ANSWER: There were 517 cities belonging to cluster 1 which represents the geographic bounds of North America and the number of "large cities" within it. 
...
km_out$centers
# using kilometers to map continents by coordinates of cities corresponding to colors
BigCities_km <- BigCities %>%
  mutate(cluster = as.character(km_out$cluster))
# plotting data and using color to differentiate continents based on location
...
> ANSWER: There are 909 cities in cluster 1 now, with only 3 delinieations instead of 6. Cluster 1 now represents the large cities within The Americas rather than defining the cities in North America and South America seperately by color using geographic coordinates to make distinctions. 
...
> ANSWER:  There are now only 291 cities in cluster 1.  This cluster represents what most maps would define as "The Middle East" and the largest cities within that using 15 distinctions with lattitude and longitude. 
